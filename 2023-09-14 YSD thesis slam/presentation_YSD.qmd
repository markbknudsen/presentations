...
Covariates $X(t)$ are also time-dependent. Can generalize to a weight *process*
$$
W_i(t) = \frac{1}{\lambda^A(\tau \mid \mathcal{F}_{\tau-})^{A(t)} \mathrm{e}^{-\Lambda^A(t \wedge \tau)}}
$$
where $\lambda^A$ is the hazard/rate of treatment initiation and $\Lambda^A(t) = \int_0^t \lambda^A(s \mid \mathcal{F}_{s-}) \mathrm{d} s$.
